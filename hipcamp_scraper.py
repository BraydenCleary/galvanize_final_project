...
def worker(id, total_thread_count):
  num_to_iterate_over = 80000 / total_thread_count
  end_value = num_to_iterate_over * id
  start_value = end_value - num_to_iterate_over
...
threads = []
worker_ids = range(1, 11)
for i in worker_ids:
    t = threading.Thread(target=worker, args=(i,len(worker_ids),))
    threads.append(t)
    t.start()