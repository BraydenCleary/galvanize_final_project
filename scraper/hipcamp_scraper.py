  ...
  def _find_activities(self, soup):
    ...
    try:
      for activity in soup.find('section', {'class': 'activities'}).find_all('div', {'class': 'feature'}):
        activities[s.snakecase(activity.find('span', {'class': 'name'}).text.strip())] = True
    except:
      activities = {}
    return activities
  ...
  def _find_other_features(self, soup):
    ...
    try:
      for feature in soup.find('section', {'class': 'other-features'}).find_all('div', {'class': 'feature'}):
        other_features[s.snakecase(feature.text.strip())] = True
    except:
      other_features = {}
  ...
  def _find_details(self, soup):
    ...
    try:
      ...
      for detail in raw_details:
        if s.snakecase(detail.split(':')[0].strip()) == 'no__of_sites':
          key = 'number_of_sites'
        else:
          key = s.snakecase(detail.split(':')[0].strip())
